...
class Event:
    def __init__(
        ...
        event_text: str,
        event_ttp: str,
        supported_webhooks: list = [],
    ) -> None:
        self.event_name = event_name
        self.event_text = event_text
        self.event_ttp = (
            Path(__file__).parents[1].joinpath("templates", event_ttp).open().read()
        )
    ...
    def parse_event(self, event_message):
        logger.debug(
            f"Attempting to process event using: {self.event_name} ({event_message})"
        )
    ...
    def process(self):
        for webhook in self.webhooks:
            ...
            if not check_attr:
                logger.error(
                    "Unable to process a Webhook due to handle_webhook method missing..."
                )
                return
